  ...
  if (active && payload && payload.length) {
    ...
    return (
      <div className="bg-slate-900 text-white px-3 py-2 rounded-lg text-xs shadow-xl border border-slate-700 font-sans z-50">
        ...
        <p className="text-slate-300">Sales: <span className="font-bold text-white">{data.amount}</span></p>
...
export default function SalesPaymentMethodChartCard({ methods = [] }) {
  if (!methods || methods.length === 0) {
    return (
      <div className="bg-white border border-slate-200/70 rounded-2xl p-5 shadow-xs flex flex-col justify-between">
        <div>
          <h3 className="text-sm font-bold text-slate-900 mb-3.5">Sales by Payment Method</h3>
          <div className="text-center py-8 text-xs text-slate-400 font-medium">
            No payment method transaction data available yet.
          </div>
        </div>
      </div>
    );
