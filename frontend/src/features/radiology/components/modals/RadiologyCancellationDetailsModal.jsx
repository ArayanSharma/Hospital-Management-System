...
export default function RadiologyCancellationDetailsModal({ order, isOpen, onClose }) {
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title="Radiology Order Cancellation Record"
    ...
    >
      <div className="space-y-4 text-xs">
        <div className="p-3.5 bg-rose-50 border border-rose-200 rounded-2xl flex items-center gap-3 text-rose-900">
          ...
          <div>
            <p className="font-extrabold text-sm text-rose-900">Radiology Order Cancelled</p>
            <p className="text-[11px] text-rose-700 font-semibold mt-0.5">
              Cancelled On: {formatDate(order.updatedAt || new Date())} ({formatTime(order.updatedAt || new Date())})
            </p>
          </div>
        </div>
        ...
        <div className="p-4 bg-white border border-slate-200 rounded-2xl space-y-1.5 shadow-2xs">
          ...
          <p className="text-slate-800 font-medium leading-relaxed bg-slate-50 p-2.5 rounded-xl border border-slate-200">
            {order.cancellationReason || order.notes || "Order cancelled by attending radiologist / physician."}
          </p>
        </div>
      </div>
    </Modal>
