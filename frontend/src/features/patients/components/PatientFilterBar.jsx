        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <button
            ...
            className={`group flex items-center gap-1.5 border font-semibold text-xs px-3.5 py-2 rounded-xl transition-all duration-150 cursor-pointer shadow-2xs active:scale-95 ${
              showMoreFilters || gender || bloodGroup || status || search
          ...
          >
            ...
            {(gender || bloodGroup || status || search) && (
        ...
        <div className="pt-3 border-t border-slate-100 flex flex-wrap items-center gap-2 animate-in fade-in slide-in-from-top-1 duration-200">
          ...
          {gender && (
            <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-lg text-[11px] font-bold bg-blue-50 text-blue-700 border border-blue-200">
              Gender: {gender}
              <button type="button" onClick={() => setGender("")} className="hover:text-blue-900 font-black cursor-pointer">×</button>
            </span>
          )}
          {bloodGroup && (
            <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-lg text-[11px] font-bold bg-rose-50 text-rose-700 border border-rose-200">
              Blood: {bloodGroup}
              <button type="button" onClick={() => setBloodGroup("")} className="hover:text-rose-900 font-black cursor-pointer">×</button>
            </span>
          )}
          {status && (
          ...
          {search && (
            <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-lg text-[11px] font-bold bg-purple-50 text-purple-700 border border-purple-200">
              Search: "{search}"
              <button type="button" onClick={() => setSearch("")} className="hover:text-purple-900 font-black cursor-pointer">×</button>
            </span>
          )}
          {!gender && !bloodGroup && !status && !search && (
            <span className="text-[11px] text-slate-400 italic font-medium">No filters active. Use dropdowns above to filter patients.</span>
          )}
        </div>
      )}
    </div>
