...
export const getPatientRegistrationReport = async ({ startDate, endDate }) => {
  const cacheKey = `hms:report:patient_reg:${startDate || "all"}:${endDate || "all"}`;
...
export const getAppointmentReport = async ({ startDate, endDate }) => {
  const cacheKey = `hms:report:appointments:${startDate || "all"}:${endDate || "all"}`;
  const { data: report } = await getOrSetCache(
    cacheKey,
    async () => {
      const dateQuery = parseDateRange(startDate, endDate);
...
export const getRevenueReport = async ({ startDate, endDate }) => {
  const cacheKey = `hms:report:revenue:${startDate || "all"}:${endDate || "all"}`;
  const { data: report } = await getOrSetCache(
    cacheKey,
    async () => {
      const dateQuery = parseDateRange(startDate, endDate);
...
export const getPharmacySalesReport = async ({ startDate, endDate }) => {
  const cacheKey = `hms:report:pharmacy_sales:${startDate || "all"}:${endDate || "all"}`;
  const { data: report } = await getOrSetCache(
    cacheKey,
    async () => {
      const dateQuery = parseDateRange(startDate, endDate);
